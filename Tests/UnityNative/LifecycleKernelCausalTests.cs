...
{
    [TestMethod]
    public void CalloutIssuanceRequiresExactNonNullRunningAttemptWithoutMutation()
    {
        var kind = LifecycleKernelHarness.New().EnumValue("KernelCalloutKind", "Cleanup");
        var noScope = LifecycleKernelHarness.New();
        var noScopeState = noScope.SnapshotFingerprint();
        noScope.Reject("InvalidToken", "IssueCalloutCapability", null, kind);
        Assert.AreEqual(noScopeState, noScope.SnapshotFingerprint());
        var adapterFailure = Assert.ThrowsException<System.Reflection.TargetInvocationException>(
            () => noScope.CreateCalloutCapability("Cleanup"));
        Assert.IsInstanceOfType<System.InvalidOperationException>(adapterFailure.InnerException);
        Assert.AreEqual(noScopeState, noScope.SnapshotFingerprint());

        var source = LifecycleKernelHarness.New();
        var request = source.AdapterCall("RequestDispose");
        source.AcceptAdapter(request);
        var capability = source.AdapterCapability(request);
        using (source.EnterAdapterScope(request))
            Assert.IsNotNull(source.CreateCalloutCapability("Cleanup"));

        source.CompleteStartedDisposal(source.AdapterReceipt(request));
        var completedState = source.SnapshotFingerprint();
        source.Reject("InvalidToken", "IssueCalloutCapability", capability, kind);
        Assert.AreEqual(completedState, source.SnapshotFingerprint());

        var foreign = LifecycleKernelHarness.New();
        var foreignState = foreign.SnapshotFingerprint();
        foreign.Reject("InvalidToken", "IssueCalloutCapability", capability, kind);
        Assert.AreEqual(foreignState, foreign.SnapshotFingerprint());
        source.AssertValid();
        foreign.AssertValid();
    }
