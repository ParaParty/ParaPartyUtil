...
internal sealed class LifecycleKernelHarness
{
    ...
    internal IDisposable EnterKernelLock()
    {
        var sync = Field(_kernel, "_sync");
        System.Threading.Monitor.Enter(sync);
        return new MonitorScope(sync);
    }

    internal object KernelField(string field) => Field(_kernel, field);
    ...
    internal static object Field(object target, string field)
    {
        Assert.IsNotNull(target);
        var info = target.GetType().GetField(field, InstanceFlags);
        Assert.IsNotNull(info, $"Missing field {target.GetType().FullName}.{field}.");
        return info.GetValue(target);
    }
