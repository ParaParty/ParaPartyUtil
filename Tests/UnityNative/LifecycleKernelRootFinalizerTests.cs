...
public class LifecycleKernelRootFinalizerTests
{
    ...
    [DataTestMethod]
    [DataRow("FinalizeWrapper", "EnterOperation", "ReleaseOperation")]
    [DataRow("FinalizeWrapper", "EnterCallback", "ReleaseCallback")]
    [DataRow("PrepareDomainShutdown", "EnterOperation", "ReleaseOperation")]
    [DataRow("PrepareDomainShutdown", "EnterCallback", "ReleaseCallback")]
    public void RecoveryHandoffWaitsForExactAdmissionDrainBeforeQuiescence(
        string handoff,
        string enter,
        string release)
    {
        ...
        if (handoff == "FinalizeWrapper")
        {
            kernel.Accept(handoff);
        }
        else
        {
            var shutdown = LifecycleKernelHarness.TaskResult(kernel.RegistryCall(
        ...
        Assert.AreEqual("Closing", kernel.State("Admission"));
        var first = LifecycleKernelHarness.TaskResult(kernel.RegistryCall(
            "DrainAsync", System.DateTime.UtcNow.AddMinutes(1)));
        Assert.AreEqual(1L, System.Convert.ToInt64(
            LifecycleKernelHarness.Property(first, "RetryableRetained")));
        Assert.AreEqual("Closing", kernel.State("Admission"));
        Assert.AreEqual(0L, kernel.Number("CompletedStageCount"));
        Assert.IsNull(kernel.KernelField("_runningStageReceipt"));
        Assert.IsNull(kernel.KernelField("_quiescenceReceipt"));
        Assert.AreEqual(1L, kernel.Number("DisposalAttemptId"));
        Assert.AreEqual("Running", kernel.State("DisposalAttempt"));

        using var start = new System.Threading.ManualResetEventSlim(false);
        var releaseTask = System.Threading.Tasks.Task.Run(() =>
        {
            start.Wait();
            return kernel.Call(release, token);
        });
        var retryTask = System.Threading.Tasks.Task.Run(() =>
        {
            start.Wait();
            return LifecycleKernelHarness.TaskResult(kernel.RegistryCall(
                "RetryAsync", owner, System.DateTime.UtcNow.AddMinutes(1)));
        });
        start.Set();
        System.Threading.Tasks.Task.WaitAll(releaseTask, retryTask);

        Assert.IsTrue(LifecycleKernelHarness.Bool(releaseTask.Result, "Accepted"));
        var concurrentRecovered = System.Convert.ToInt64(
            LifecycleKernelHarness.Property(retryTask.Result, "Recovered"));
        var concurrentRetained = System.Convert.ToInt64(
            LifecycleKernelHarness.Property(retryTask.Result, "RetryableRetained"));
        Assert.AreEqual(1L, concurrentRecovered + concurrentRetained);
        if (concurrentRecovered == 0)
        {
            var final = LifecycleKernelHarness.TaskResult(kernel.RegistryCall(
                "RetryAsync", owner, System.DateTime.UtcNow.AddMinutes(1)));
            Assert.AreEqual(1L, System.Convert.ToInt64(
                LifecycleKernelHarness.Property(final, "Recovered")));
        }
    ...
    [DataRow("RequestTransfer")]
    public void ShutdownFlagWinsEveryPreparingTransferTransition(string operation)
    {
        var kernel = LifecycleKernelHarness.NewWithRegistry();
        var owner = kernel.AdapterCall("RequestTransfer");
        kernel.AcceptAdapter(owner);
        var waiter = kernel.AdapterCall("RequestTransfer");
        kernel.AcceptAdapter(waiter);
        System.Threading.Tasks.Task<object> shutdown;
        object lateRequest = null;
        ...
        using (kernel.EnterKernelLock())
        {
            ...
            Assert.IsTrue(System.Threading.SpinWait.SpinUntil(
                () => kernel.RegistryFlag("Shutdown"), 5000));
            var shutdownWon = kernel.SnapshotFingerprint();
            if (operation == "RequestTransfer")
            {
                lateRequest = kernel.AdapterCall(operation);
                kernel.RejectAdapter("IllegalState", lateRequest);
                Assert.IsFalse((bool)LifecycleKernelHarness.Property(lateRequest, "IsJoin"));
            }
            else
            {
                kernel.Reject("IllegalState", operation, kernel.AdapterReceipt(owner));
            }
            Assert.AreEqual(shutdownWon, kernel.SnapshotFingerprint());
        }

        var result = LifecycleKernelHarness.TaskResult(shutdown.Result);
        Assert.AreEqual(1L, System.Convert.ToInt64(
            LifecycleKernelHarness.Property(result, "Recovered")));
        Assert.AreEqual("transfer-abort:1", kernel.ObserveAdapterRequest(waiter));
        if (lateRequest != null)
            Assert.IsNull(kernel.ObserveAdapterRequest(lateRequest));
        Assert.AreEqual(1L, kernel.Number("TransferRecordCount"));
        Assert.AreEqual("Disposed", kernel.State("Life"));
        kernel.AssertValid();
    }
