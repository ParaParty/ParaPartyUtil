    ...
    internal sealed class LifecycleKernel
    {
            ...
            KernelCalloutCapability callout)
        {
            lock (_sync)
            {
                ...
                if (ReferenceEquals(_disposeCapability, currentAttempt))
            ...
            KernelCalloutCapability callout)
        {
            lock (_sync)
            {
                ...
                if (ReferenceEquals(_rollbackCapability, currentAttempt))
        ...
        internal KernelOrphanRecoveryOutcome TryRecoverOrphan()
        {
            try
            {
                ...
                var disposeReceipt = ((KernelAttemptReservation)start.Value).Receipt;
                if (Snapshot().Admission == KernelAdmission.Closing &&
                    !CloseAdmission(disposeReceipt).Accepted)
                    return KernelOrphanRecoveryOutcome.RetryableRetained;

                KernelReceipt quiescenceReceipt = null;
                if (!RecoverSimpleStage(KernelStage.Managed, disposeReceipt))
                    return KernelOrphanRecoveryOutcome.RetryableRetained;
                if (!StageComplete(KernelStage.NativeQuiesce))
                {
                    var quiesceStage = BeginStage(KernelStage.NativeQuiesce, disposeReceipt);
                    if (!quiesceStage.Accepted)
                        return KernelOrphanRecoveryOutcome.RetryableRetained;
                    var quiescence = RecordQuiescence((KernelReceipt)quiesceStage.Value);
                    if (!quiescence.Accepted)
                        return KernelOrphanRecoveryOutcome.RetryableRetained;
                    quiescenceReceipt = quiescence.Value as KernelReceipt;
                }
                if (!RecoverSimpleStage(KernelStage.CallbackFence, disposeReceipt))
                    return KernelOrphanRecoveryOutcome.RetryableRetained;
                if (!StageComplete(KernelStage.NativeAuthority))
                {
                    var nativeStage = BeginStage(KernelStage.NativeAuthority, disposeReceipt);
                    if (!nativeStage.Accepted ||
                        !CommitNativeAuthority(
                            (KernelReceipt)nativeStage.Value,
                            quiescenceReceipt).Accepted)
                        return KernelOrphanRecoveryOutcome.RetryableRetained;
                }
        ...
        private KernelOrphanRecoveryOutcome RecoverOrphanTicket(LifecycleKernelSnapshot snapshot)
        {
            ...
            if (!start.Accepted)
                return start.Code == KernelTransitionCode.StableFailure
                    ? KernelOrphanRecoveryOutcome.StableRetained
                    : KernelOrphanRecoveryOutcome.RetryableRetained;
            var rollback = ((KernelAttemptReservation)start.Value).Receipt;
            return CompleteRollback(rollback, KernelAttemptOutcome.Success).Accepted
                ? KernelOrphanRecoveryOutcome.Recovered
                : KernelOrphanRecoveryOutcome.RetryableRetained;
        }
        ...
        private bool RecoverSimpleStage(KernelStage stage, KernelReceipt disposeReceipt)
        {
            ...
            if (!reservation.Accepted)
                return false;
            var stageReceipt = (KernelReceipt)reservation.Value;
            switch (stage)
            {
                case KernelStage.Managed:
        ...
        private bool RecoverBaseResources(KernelReceipt disposeReceipt)
        {
            if (StageComplete(KernelStage.BaseResources))
                return true;
            var reservation = BeginStage(KernelStage.BaseResources, disposeReceipt);
            if (!reservation.Accepted)
                return false;
            var stageReceipt = (KernelReceipt)reservation.Value;
